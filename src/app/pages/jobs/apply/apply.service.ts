...
import {JobApplydata} from './data';
import {DecimalPipe} from '@angular/common';
import {debounceTime, delay, switchMap, tap} from 'rxjs/operators';
import {SortColumn, SortDirection} from './apply-sortable.directive';
...
function sort(jobApply: jobApplyModel[], column: SortColumn, direction: string): jobApplyModel[] {
  if (direction === '' || column === '') {
    return jobApply;
  } else {
...
function matches(country: jobApplyModel, term: string, pipe: PipeTransform) {
...
export class JobApplyService {
  ...
  private jobLlist$ = new BehaviorSubject<jobApplyModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let jobApply = sort(JobApplydata, sortColumn, sortDirection);    
