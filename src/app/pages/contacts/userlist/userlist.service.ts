...
import {SortColumn, SortDirection} from './userlist-sortable.directive';

interface SearchResult {
  countries: userListModel[];
  total: number;
}
...
function sort(countries: userListModel[], column: SortColumn, direction: string): userListModel[] {
  if (direction === '' || column === '') {
    return countries;
  } else {
...
function matches(country: userListModel, term: string, pipe: PipeTransform) {
...
export class userListService {
  ...
  private _countries$ = new BehaviorSubject<userListModel[]>([]);
  ...
  private _search(): Observable<SearchResult> {
    ...
    // 1. sort
    let countries = sort(userList, sortColumn, sortDirection);    
