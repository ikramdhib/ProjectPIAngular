...
export class UserServiceService {
  ...
  getStudentsBySupervisor(encadrantId: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/api/v1/user/students/${encadrantId}`).pipe(
      map((students: any[]) => {
  ...
  validateStudent(studentId: string) {
    return this.http.put(`${this.apiUrl}/api/v1/user/${studentId}/validate`, {});
  }

  getStudentsByAllStages(serviceId: string): Observable<any[]> {
    return this.http.get<any[]>(`${this.apiUrl}/api/v1/user/stages/students/${serviceId}`);
  }
  rejectStudent(studentId: string, rejectionReason: string): Observable<any> {
    const url = `${this.apiUrl}/${studentId}/api/v1/user/reject/${rejectionReason}`;
